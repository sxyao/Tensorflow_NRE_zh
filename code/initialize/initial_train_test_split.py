...
def init():
    ...
    f = open('./origin_data_zh/all_clean_sim1_noredun_wsib.txt', 'r')
    ...
    # split 20% data(one example is a group of sentence for one pair of entity and a relation )
    label_groupofsen = defaultdict(list)
    for i in xrange(len(train_x)):
        label_groupofsen[tuple(train_y[i])].append(train_x[i])

    print 'number of labels', len(label_groupofsen)
    print label_groupofsen.keys()
    train_x_n = []
    train_y_n = []
    test_x_n = []
    test_y_n = []

    for k in label_groupofsen:
        print np.argmax(np.array(k)), 'number of entity pair/group of sentence', int(
            len(label_groupofsen[k]) * 0.2), '/', len(label_groupofsen[k])
        label_groupofsen[k] = np.array(label_groupofsen[k])
        idxli = random.sample(range(len(label_groupofsen[k])), int(len(label_groupofsen[k]) * 0.2))
        for i in range(len(label_groupofsen[k])):
            if i in idxli:
                test_x_n.append(label_groupofsen[k][i])
                test_y_n.append(k)
            else:
                train_x_n.append(label_groupofsen[k][i])
                train_y_n.append(k)

    train_x = np.array(train_x_n)
    train_y = np.array(train_y_n)
    test_x = np.array(test_x_n)
    test_y = np.array(test_y_n)

    np.save('./data/vec1_newtvsplit.npy', vec)
    np.save('./data/train_x_newtvsplit.npy', train_x)
    np.save('./data/train_y_newtvsplit.npy', train_y)
    np.save('./data/testall_x_newtvsplit.npy', test_x)
    np.save('./data/testall_y_newtvsplit.npy', test_y)
    ...
    np.save('./data/pone_test_x_newtvsplit.npy', pone_test_x)
    np.save('./data/pone_test_y_newtvsplit.npy', pone_test_y)
    np.save('./data/ptwo_test_x_newtvsplit.npy', ptwo_test_x)
    np.save('./data/ptwo_test_y_newtvsplit.npy', ptwo_test_y)
    np.save('./data/pall_test_x_newtvsplit.npy', pall_test_x)
    np.save('./data/pall_test_y_newtvsplit.npy', pall_test_y)


def seperate():
    print 'reading training data'
    x_train = np.load('./data/train_x_newtvsplit.npy')
    ...
    np.save('./data/train_word_newtvsplit.npy', train_word)
    np.save('./data/train_pos1_newtvsplit.npy', train_pos1)
    np.save('./data/train_pos2_newtvsplit.npy', train_pos2)

    print 'reading p-one test data'
    x_test = np.load('./data/pone_test_x_newtvsplit.npy')
    ...
    np.save('./data/pone_test_word_newtvsplit.npy', test_word)
    np.save('./data/pone_test_pos1_newtvsplit.npy', test_pos1)
    np.save('./data/pone_test_pos2_newtvsplit.npy', test_pos2)

    print 'reading p-two test data'
    x_test = np.load('./data/ptwo_test_x_newtvsplit.npy')
    ...
    test_pos1 = np.array(test_pos1)
    test_pos2 = np.array(test_pos2)
    np.save('./data/ptwo_test_word_newtvsplit.npy', test_word)
    np.save('./data/ptwo_test_pos1_newtvsplit.npy', test_pos1)
    np.save('./data/ptwo_test_pos2_newtvsplit.npy', test_pos2)

    print 'reading p-all test data'
    x_test = np.load('./data/pall_test_x_newtvsplit.npy')
    ...
    test_pos1 = np.array(test_pos1)
    test_pos2 = np.array(test_pos2)
    np.save('./data/pall_test_word_newtvsplit.npy', test_word)
    np.save('./data/pall_test_pos1_newtvsplit.npy', test_pos1)
    np.save('./data/pall_test_pos2_newtvsplit.npy', test_pos2)

    print 'seperating test all data'
    x_test = np.load('./data/testall_x_newtvsplit.npy')
    ...
    np.save('./data/testall_word_newtvsplit.npy', test_word)
    np.save('./data/testall_pos1_newtvsplit.npy', test_pos1)
    np.save('./data/testall_pos2_newtvsplit.npy', test_pos2)


def getsmall():
    print 'reading training data'
    word = np.load('./data/train_word_newtvsplit.npy')
    pos1 = np.load('./data/train_pos1_newtvsplit.npy')
    pos2 = np.load('./data/train_pos2_newtvsplit.npy')
    y = np.load('./data/train_y_newtvsplit.npy')
    ...
    np.save('./data/small_word_newtvsplit.npy', new_word)
    np.save('./data/small_pos1_newtvsplit.npy', new_pos1)
    np.save('./data/small_pos2_newtvsplit.npy', new_pos2)
    np.save('./data/small_y_newtvsplit.npy', new_y)


# get answer metric for PR curve evaluation
def getans():
    test_y = np.load('./data/testall_y_newtvsplit.npy')
    eval_y = []
    for i in test_y:
        eval_y.append(i[1:])
    allans = np.reshape(eval_y, (-1))
    np.save('./data/allans_newtvsplit.npy', allans)
