...
import datetime
import os
import network_att_rel_sen_same_en as network
        ...
        with sess.as_default():
            ...
            for one_epoch in range(settings.num_epochs):
                ...
                for i in range(int(len(temp_order) / float(settings.big_num))):
                    ...
                    if current_step >= 10000 and current_step % 1000 == 0:
                        # if current_step == 50:
                        print 'saving model'
                        path = saver.save(sess, save_path + 'ATT_GRU_model_sensame', global_step=current_step)
