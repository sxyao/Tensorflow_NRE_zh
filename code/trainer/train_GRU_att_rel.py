...
import datetime
import os
import network_att_rel_sena_diff as network
...
def main(_):
    ...
    train_y = np.load('./data/small_y_ner1.npy')
    train_word = np.load('./data/small_word_ner1.npy')
    train_pos1 = np.load('./data/small_pos1_ner1.npy')
    train_pos2 = np.load('./data/small_pos2_ner1.npy')
        ...
        with sess.as_default():
            ...
            def train_step(word_batch, pos1_batch, pos2_batch, y_batch, big_num):
                ...
                for i in range(len(word_batch)):
                    ...
                    tem1 = np.array(range(0, settings.num_steps * len(word_batch[i])))
                    total_y += list(tem1 * settings.num_classes + np.argmax(y_batch[i]) + len(total_y))
                ...
                feed_dict[m.input_y_expanded] = total_y
            ...
            for one_epoch in range(settings.num_epochs):
                ...
                for i in range(int(len(temp_order) / float(settings.big_num))):
                    ...
                    if current_step >= 8000 and current_step % 1000 == 0:
                        # if current_step == 50:
                        print 'saving model'
                        path = saver.save(sess, save_path + 'ATT_GRU_model_ner1_ep7_senadiff_', global_step=current_step)
