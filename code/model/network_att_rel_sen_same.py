...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        for i in range(big_num):
            ...
            cur_sen_r = tf.reshape(relation_embedding[tf.cast(rel, tf.int32), :], [gru_size, 1])
            sen_alpha.append(
                tf.reshape(tf.nn.softmax(tf.reshape(tf.matmul(tf.mul(sen_repre[i], sen_a), cur_sen_r), [batch_size])),
                           [1, batch_size]))
