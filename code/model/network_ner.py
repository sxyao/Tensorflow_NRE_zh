...
class Settings(object):
    def __init__(self):
        ...
        self.big_num = 30
        self.ner_size = 5
        self.ner_num = 25 #TODO: modify the number of different ner tags by initial output
...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        self.input_ner = tf.placeholder(dtype=tf.int32, shape=[None, num_steps], name='input_ner')
        ...
        ner_embedding = tf.get_variable('ner_embedding', [settings.ner_num, settings.ner_size])
        ...
        inputs_forward = tf.concat(2, [tf.nn.embedding_lookup(word_embedding, self.input_word),
                                       ...
                                       # tf.nn.embedding_lookup(ner_embedding, self.input_ner)])
        inputs_backward = tf.concat(2,
                                    [tf.nn.embedding_lookup(word_embedding, tf.reverse(self.input_word, [False, True])),
                                     ...
                                     # tf.nn.embedding_lookup(ner_embedding, tf.reverse(self.input_ner, [False, True]))])
        ...
        for i in range(big_num):
            ...
            topkvalues, topkindices = tf.nn.top_k(tf.add(
                tf.reshape(tf.matmul(relation_embedding, tf.reshape(sen_repre[i], [gru_size, batch_size])),
                           [self.num_classes, batch_size]), tf.reshape(sen_d, [self.num_classes, 1])), 5)
