...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        self.input_y_expanded = tf.placeholder(dtype=tf.int32, shape=[None], name='input_y_expanded')
        ...
        attention_w = tf.get_variable('attention_omega', [gru_size, self.num_classes])
        sen_a = tf.get_variable('attention_A', [gru_size, self.num_classes])
        sen_r = tf.get_variable('query_r', [gru_size, self.num_classes])
        ...
        # cur_attention_w = tf.gather(attention_w, tf.argmax(self.input_y, 0, name='find_relation'))
        attention_r = tf.reshape(tf.batch_matmul(tf.reshape(tf.nn.softmax(
            tf.reshape(
                tf.gather(tf.reshape(
                    tf.matmul(tf.reshape(tf.tanh(output_h), [total_num * num_steps, gru_size]), attention_w),
                    [total_num*num_steps*self.num_classes, 1]),
                    self.input_y_expanded, validate_indices=True, name='find_relation'),
        ...
        for i in range(big_num):
            ...
            rel = tf.argmax(self.input_y[i, :], 0)
            cur_sen_r = tf.reshape(sen_r[:, tf.cast(rel, tf.int32)], [gru_size, 1])
            cur_sen_a = tf.reshape(sen_a[:, tf.cast(rel, tf.int32)], [gru_size])

            sen_alpha.append(
                tf.reshape(tf.nn.softmax(tf.reshape(tf.matmul(tf.mul(sen_repre[i], cur_sen_a), cur_sen_r), [batch_size])),
                           [1, batch_size]))
            ...
            tf.reshape(sen_repre[i], [gru_size, batch_size])
            topkvalues, topkindices = tf.nn.top_k(tf.nn.softmax(tf.add(
                tf.reshape(tf.matmul(relation_embedding, tf.reshape(sen_repre[i], [gru_size, batch_size])),
                           [self.num_classes, batch_size]), tf.reshape(sen_d, [self.num_classes, 1]))),
                tf.minimum(5, batch_size))
            self.prov_values.append(topkvalues)
            self.prov_indices.append(topkindices)

            # self.prov.append(tf.argmax(tf.add(
            #     tf.reshape(tf.matmul(relation_embedding, tf.reshape(sen_repre[i], [gru_size, batch_size])),
            #                [self.num_classes, batch_size]), tf.reshape(sen_d, [self.num_classes, 1])), 1))
