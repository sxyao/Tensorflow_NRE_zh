...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        for i in range(big_num):
            ...
            sen_alpha.append(
                tf.transpose(tf.nn.softmax(tf.matmul(tf.mul(sen_repre[i], sen_a), sen_r), 0)))

            sen_s.append(tf.reshape(tf.matmul(sen_alpha[i], sen_repre[i]), [self.num_classes, gru_size]))
            sen_out.append(tf.add(tf.reshape(tf.reduce_sum(tf.mul(relation_embedding, sen_s[i]), 1), [self.num_classes]), sen_d))
